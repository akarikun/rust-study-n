<template>
    <f7-page :page-content="false">
        ...
        <f7-tabs class="tab" animated swipeable @tab:show="tab_show">
            <f7-tab :id="`tab-n3-1000-${item.id}`" v-for="(item, index) in form" :key="item.id" class="page-content">
                <f7-card outline-md>
        ...
        <f7-sheet class="demo-sheet-swipe-to-close" style="height: auto" swipe-to-close push backdrop>
            ...
            <f7-page-content>
                <f7-card outline-md>
                    <f7-card-content>
                        <f7-block strong v-html="modal_form.content"></f7-block>
                        <br /><br /><br /><br /><br />
                        <f7-list strong-ios outline-ios dividers-ios>
                            <f7-list-item v-for="({ text, result }, index2) in format_result(modal_form)" :key="index2">
                                <f7-button large fill>{{ text }}</f7-button>
                            </f7-list-item>
                        </f7-list>
                    </f7-card-content>
                </f7-card>
...
const tab_show = (e) => {
    ...
    id = parseInt(id) - 1;
    modal_form.value = form.filter(x => x.id == id)[0];
}
...
const handle_select = (result, sel) => {
    // debugger
}
